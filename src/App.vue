...
import {
  rebalanceItemA, viewLabelHelper, labelItemHelper, moveItemHelper, deleteItemHelper, defaultItemA,
} from './lib';
...
function normalShortcuts(e, posA, itemA, dispA, viewLabelA, dispatch) {
  ...
  } else if ([49, 50, 51].indexOf(e.keyCode) !== -1 && e.shiftKey) { // Shift-1/2/3
    itemA0 = labelItemHelper(e.keyCode - 48, itemA0, posA0, 0);
    r = rebalanceItemA([], [], itemA0, viewLabelA);
    dispatch('updateItemA', { itemA: r.itemA, dispA: r.dispA });
  } else if ([49, 50, 51].indexOf(e.keyCode) !== -1 && !e.shiftKey) { // 1/2/3
    const viewLabelA0 = viewLabelHelper(viewLabelA, e.keyCode - 48);
    dispatch('updateViewLabelA', viewLabelA0);
    dispatch('updatePosA', [0]);
    r = rebalanceItemA([], [], itemA0, viewLabelA0);
    dispatch('updateItemA', { itemA: r.itemA, dispA: r.dispA });
  }
}
...
function selectedShortcuts(e, posA, itemA, modesub, viewLabelA, dispatch) {
  ...
  if (e.keyCode === 38) { // up
    ...
    if (modesub === 'move') {
      const newViewLabelA = [];
      dispatch('updateViewLabelA', newViewLabelA);
      const r = rebalanceItemA([], [], itemA0, newViewLabelA);
      dispatch('updateItemA', { itemA: r.itemA, dispA: r.dispA });
      ...
      const r = rebalanceItemA([], [], itemA0, viewLabelA);
      dispatch('updateItemA', { itemA: r.itemA, dispA: r.dispA });
    }
  }
}
    ...
    if (window.$cookies.get('TokenString')) {
      ...
      axios.post(`${process.env.VUE_APP_AXIOS}refresh`, params)
        .then((response) => {
          ...
          const r = rebalanceItemA(
            [], [], JSON.parse(response.data.ItemA), this.$store.state.viewLabelA,
          );
      ...
      this.$store.dispatch('updateUID', -1);
      const r = rebalanceItemA([], [], defaultItemA(), this.$store.state.viewLabelA);
      this.$store.dispatch('updateItemA', { itemA: r.itemA, dispA: r.dispA });
